...
t.sml.sum <- t.sml %>%
  ...
  mutate(Classification = as.factor(case_when(p_adj < 0.05 & FC > 0 ~ "Increased",
                                    p_adj < 0.05 & FC < 0 ~ "Decreased",
                                    TRUE ~ "Unchanged"))) %>%
  mutate(Classification = fct_relevel(Classification, c("Increased", "Unchanged", "Decreased"))) %>%
