...
count.plot <- ggplot(barplot.dat, aes(x = behavior, fill = sig.bact.cor)) + 
  ...
  scale_fill_manual(values = c("aliceblue", "steelblue")) +
...
###plot volcano plot
volcano.plot <- ggplot() +
  geom_vline(xintercept = log2(1.25), size = 0.25, linetype = "dashed") +
  geom_vline(xintercept = log2(0.75), size = 0.25, linetype = "dashed") +
  geom_hline(yintercept = -log10(0.05), size = 0.25, linetype = "dashed") +
  geom_point(data = not.sig.mfs, aes(x = mean.log2.fc, y = -log10(p)), shape = 21, size = 3, fill = "darkgray", color = "black", stroke = 0.1, alpha = 0.65) +
  geom_point(data = sig.mfs, aes(x = mean.log2.fc, y = -log10(p), fill = mean.log2.fc), size = 3, color = "black", stroke = 0.1, shape = 21) +
