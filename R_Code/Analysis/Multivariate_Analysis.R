...
umap.custom <- umap.defaults
umap.custom$n_neighbors = 15
umap.custom$min_dist = 0.05
...
####
MF.umap <- umap(UMAP.dat, config = umap.custom)
summary(MF.umap)


####
MF.umap.proj <- data.frame(MF.umap$layout) %>%
  rownames_to_column(var = "MF")# %>%
# filter(!X1 > 20)
#MF.umap.proj <- left_join(MF.umap.proj, clust.results)
#MF.umap.proj <- left_join(MF.umap.proj, dat.cor)

#MF.umap.proj <- left_join(MF.umap.proj, qc2.count) %>%
#  filter(!is.na(Name))

#####
ggplot(MF.umap.proj, aes(x=X1, y=X2,)) +
  geom_point(alpha = 0.6) +
  theme_test()


####Metabolite correlation data
metab.cor.dat <- qc2.dat %>%
  group_by(MF) %>%
  mutate(Pro.cor = cor(x = Adjusted_Area, y = Pro, method = "spearman", use="na.or.complete")) %>%
  mutate(BacTot.cor = cor(x = Adjusted_Area, y = BacTot, method = "spearman", use="na.or.complete")) %>%
  mutate(BacLrg.cor = cor(x = Adjusted_Area, y = BacLrg, method = "spearman", use="na.or.complete")) %>%
  mutate(BacSml.cor = cor(x = Adjusted_Area, y = BacSml, method = "spearman", use="na.or.complete")) %>%
  mutate(Phage.cor = cor(x = Adjusted_Area, y = Phage, method = "spearman", use="na.or.complete")) %>%
  select(MF, Pro.cor, BacTot.cor, BacLrg.cor, BacSml.cor, Phage.cor) %>%
  unique()
# mutate(Grz.cor = cor(x = Adjusted_Area, y = Grz, method = "spearman", use="na.or.complete")) 

UMAP.proj.2 <- left_join(MF.umap.proj, metab.cor.dat)

ggplot(UMAP.proj.2, aes(x=X1, y=X2,
                        color = Phage.cor)) +
