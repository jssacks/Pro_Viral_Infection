...
###
# practice.dat <-  metab.meta.dat%>%
#   mutate(vol.norm.area = (Adjusted_Area+1)/Vol.filt.mL,
#          pro.norm.area = vol.norm.area/Pro.Carbon)
# 
# suc.pract.dat <- practice.dat %>%
#  # filter(Name == "Sucrose") %>%
#   unite(c("treatment", "time"), col = "treatment_time", remove = FALSE) %>%
# #  filter(treatment_time %in% c("LV_36", "HV_12", "HV_24")) %>%
#   filter(Name == "2-O-alpha-D-Glucosylglycerol") %>%
# #  filter(Name == "Sucrose") %>%
#   filter(time %in% c(0, 12, 24, 36, 48)) %>%
#   mutate(HVI = case_when(treatment_time %in% c("LV_36", "HV_12", "HV_24") ~ TRUE,
#                          TRUE ~ FALSE)) %>%
#   filter(!is.na(pro.norm.area)) %>%
#   mutate(treatment = as.factor(treatment),
#          biorep = as.factor(biorep),
#          time = as.factor(time)) 
# 
# 
# #mod.out <- lme(pro.norm.area ~ treatment, data = suc.pract.dat)
# 
# x <- lmer(pro.norm.area ~ HVI + time + (1|treatment) + (1|biorep), data = suc.pract.dat)
# 
...
# #y2 <- TukeyHSD(y)
# 
# z <- emmeans(x, pairwise ~ HVI)
# summary(z)
# 
# 
# library(emmeans)
# 
# anova.out <- aov(pro.norm.area ~ treatment+(1+biorep+time), data = suc.pract.dat)
# 
# anova.rm <- aov(pro.norm.area ~ treatment*time + Error(biorep/time), data = suc.pract.dat)
# summary(anova.rm)
# 
# TukeyHSD(anova.rm)
# 
# emmeans(anova.rm, pairwise ~ treatment | time)
# 
# 
# glm.out <- glm(pro.norm.area ~ treatment*time+biorep, data = suc.pract.dat)
# summary(glm.out)
# 
# glm.anova <- aov(glm.out)
# summary(glm.anova)

# Fold change analysis normalized to Pro Carbon data  ---------------------
#####

#calculate fold change
FC.Carbon.dat <- metab.meta.dat%>%
  mutate(vol.norm.area = (Adjusted_Area+1)/Vol.filt.mL,
         pro.norm.area = vol.norm.area/Tot.C.nM) %>%
...
###write to csv
write_csv(x, file = "Intermediates/virally_altered_mfs.csv")




####
ggplot(fc.bcorr.dat, aes(x = mean.log2.fc, y = -log10(p), color = bact.cor.pearson)) +
  geom_point(alpha = 0.7, size = 1.5) +
  scale_color_gradient2(low = "darkred", mid = "gray", high = "darkblue") +
  theme_classic() +
  theme(legend.position = "bottom") +
  xlab("Mean Log2 Fold Change (Virus/Control)") +
  ylab("-Log10(p-vlaue)") + 
...
#####Just known data
focus.dat <- fc.bcorr.dat %>%
  filter(MF %in% fc.mfs$MF) %>%
  filter(!MF %in% bact.corr.mfs$MF) %>%
  filter(!Name == "Unknown") %>%
  filter(!Name == "(3-Carboxypropyl)trimethylammonium") %>%
  filter(!Name == "Betonicine") 

ggplot(focus.dat, aes(x = mean.log2.fc, y = reorder(Name, mean.log2.fc))) +
  geom_segment(aes(xend = 0, yend= reorder(Name, mean.log2.fc)), size = 0.6, color = "black") + 
  geom_point(aes(size = -log10(p)), shape = 21, color = "black", stroke = 0.5, fill = "orange") +
  ylab("Compound") +
  xlab("Mean Log2 Fold Change (Virus/Control)") +
  theme_bw() +
  theme(axis.text = element_text(size = 13, color = "black"),
...
x <- fc.bcorr.dat %>%
  mutate(behavior = case_when(signif == TRUE & mean.log2.fc <= log2(0.75) ~ "Depleated",
                              signif == TRUE & mean.log2.fc >= log2(1.25) ~ "Enriched",
                              TRUE ~ "Not Signficantly Different")) %>%
  mutate(sig.bac.cor = case_when(bact.cor.pearson > 0 & bactlm.p.adj <= 0.05 ~ "Pos_Sig"))

x.sum <- x %>%
  group_by(behavior, sig.bac.cor) %>%
  summarize(count = n())

x.nobactcor <- x %>%
  filter(is.na(sig.bac.cor))

##
ggplot(x, aes(x = behavior)) +
  geom_bar(width = 0.6, color = "black", size = 0.1, fill = "darkblue") +
  theme_test() +
  ylim(0, 350) +
  theme(axis.text = element_text(size = 13, color = "black"),
        axis.title = element_text(size = 14))
...
##
ggplot(x.nobactcor, aes(x = behavior)) +
  geom_bar(width = 0.6, color = "black", size = 0.1, fill = "steelblue") +
  theme_test() +
  ylim(0, 350) +
  theme(axis.text = element_text(size = 13, color = "black"),
        axis.title = element_text(size = 14))
...
ad.fc <- left_join(annotation.dat, x.nobactcor) %>%
  filter(signif == TRUE)
