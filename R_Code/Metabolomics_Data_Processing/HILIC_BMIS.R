...
samp.key.file <- "Meta_Data/SampleList_Josh_MortalityExperiment.csv"
is.dat.file.1 <- "Data_Raw/Skyline/ProMo_Particulate_HILIC_Pos_File1.csv"
is.dat.file.2 <- "Data_Raw/Skyline/ProMo_Particulate_HILIC_Pos_File2.csv"
is.dat.file.3 <- "Data_Raw/Skyline/ProMo_Particulate_HILIC_Neg_File1.csv"
is.dat.file.4 <- "Data_Raw/Skyline/ProMo_Particulate_HILIC_Neg_File2.csv"
hilic.dat.file <- "Intermediates/ProMo_HILIC_QC1_output.csv"
...
#Import sample key----
samp.key.old <- read_csv(samp.key.file, skip = 1) %>%
...
hilic.dat <- read_csv(hilic.dat.file) %>%
  rename("MF" = ID) %>%
...
is.dat.3<- sky_read(is.dat.file.3) %>%
  rename(SampID = `Rep`,
         MF = `Compound`) %>% 
  select(SampID, MF, Area) %>%
  filter(str_detect(.$MF, "13C") | str_detect(.$MF, "2H") | str_detect(.$MF, "15N"))

is.dat.4<- sky_read(is.dat.file.4) %>%
  rename(SampID = `Rep`,
         MF = `Compound`) %>% 
  select(SampID, MF, Area) %>%
  filter(str_detect(.$MF, "13C") | str_detect(.$MF, "2H") | str_detect(.$MF, "15N"))

is.dat.full <- rbind(is.dat.1, is.dat.2, is.dat.3, is.dat.4) %>%
  filter(!str_detect(.$SampID, "Std")) %>%
  filter(!str_detect(.$SampID, "DDA")) %>%
  filter(!str_detect(.$SampID, "Blk")) 

#save compiled IS data as a csv
write_csv(is.dat.full, file = "Intermediates/ProMo_Particulate_HILIC_ISdat.csv")
...
#  filter(!str_detect(MF, "Dihydroxypropane")) %>%
 # filter(!str_detect(MF, "Pyridinedicarboxylic")) %>%
 # filter(!str_detect(MF, "Dihydroxybenzoic")) %>%
  filter(!str_detect(MF, "Sulfolactate")) %>%
  filter(!str_detect(MF, "AMP")) %>%
  filter(!str_detect(MF, "GMP")) %>%
 # filter(!str_detect(MF, "Cys-Gly")) %>%
 ...
 # filter(!str_detect(MF, "Trimethyl-L")) %>%
...
IS.dat.key.check <- read_csv(hilic.dat.file) %>%
  rename("MF" = ID) %>%
  select(MF, Name) %>%
  filter(!Name == "Unknown") %>%
  unique() %>%
...
write_csv(BMIS_normalizedData.2, "Intermediates/ProMo_Particulate_HILIC_BMISed_dat.csv")
